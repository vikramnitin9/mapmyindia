...
for i,row in df.iterrows():
	...
	for j in range(-width_b, width-20, width_step):
		for k in range(-height_b, height-20, height_step):
			im_crop = im.crop((left + j, top + k, left + j + 60, top + k + 60))
			im_crop = im_crop.resize(resized_shape)
			im_crop.save('./data/1/'+(str)(count1)+'.jpg')
			count1 += 1
